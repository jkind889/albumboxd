...
const MODES = new Set(["plan", "execute", "inventory", "dry-run", "validate", "apply", "verify"]);
...
function parseArguments(argv = []) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    const mode = typeof flag === "string" && flag.startsWith("--") ? flag.slice(2) : "";
    if (MODES.has(mode)) {
    ...
    if (valueKeys.has(flag)) {
      if (suppliedValues.has(flag)) {
      ...
      const key = valueKeys.get(flag);
      result[key] = value;
      suppliedValues.add(flag);
      index += 1;
      continue;
    }
    throw new LegacyMigrationError(`Unknown argument: ${flag}`, "INVALID_ARGUMENTS");
  }
  if (suppliedValues.has("--overrides") && !["plan", "dry-run"].includes(result.mode)) {
    throw new LegacyMigrationError("--overrides is only valid with --plan or --dry-run", "INVALID_ARGUMENTS");
  }
  if (suppliedValues.has("--plan-sha256") && !["execute", "apply", "verify"].includes(result.mode)) {
    throw new LegacyMigrationError("--plan-sha256 is only valid with --execute, --apply, or --verify", "INVALID_ARGUMENTS");
  }
  if (suppliedValues.has("--confirm-target") && !["execute", "apply"].includes(result.mode)) {
    throw new LegacyMigrationError("--confirm-target is only valid with --execute or --apply", "INVALID_ARGUMENTS");
  }
  if (!result.help && !result.runDir) throw new LegacyMigrationError("--run-dir is required", "INVALID_ARGUMENTS");
  if (!result.help && ["apply", "execute"].includes(result.mode) && (!result.planSha256 || !result.confirmTarget)) {
    throw new LegacyMigrationError(`--${result.mode} requires --plan-sha256 and --confirm-target`, "INVALID_ARGUMENTS");
  }
  if (!result.help && result.mode === "verify" && !result.planSha256) {
