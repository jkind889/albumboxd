...
function cleanBoard(row, id, forceDefault = row.isDefault) {
  return {
    _id: id,
...
function buildBoards(source, target) {
  const existingById = currentById(target.boards);
  const defaults = currentDefaultBoards(target.boards);
  const boardMap = new Map();
  const documents = new Map(target.boards.map((board) => [idString(board._id), board]));
  ...
  for (const row of sourceBoards) {
    ...
    if (!documents.has(idString(targetBoard._id))) documents.set(idString(targetBoard._id), cleanBoard(row, targetBoard._id, targetBoard.isDefault));
  }
...
function transformReviews(source, maps, target) {
  const documents = new Map(target.reviews.map((row) => [idString(row._id), row]));
...
function transformNotifications(source, reviewMap, target) {
  const documents = new Map();
  for (const row of target.notifications || []) {
  ...
  for (const row of source.notifications || []) {
    const base = { _id: objectId(row._id, new ObjectId()), recipientUserId: text(row.recipientUserId), actorUserId: text(row.actorUserId), type: row.type, readAt: row.readAt ? dateValue(row.readAt) : null, createdAt: dateValue(row.createdAt), updatedAt: dateValue(row.updatedAt, dateValue(row.createdAt)) };
