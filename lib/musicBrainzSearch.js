...
function buildSearchUrl(query, limit = DEFAULT_LIMIT, baseUrl = MUSICBRAINZ_BASE_URL) {
  const normalizedQuery = normalizeSearchQuery(query);
  const normalizedLimit = normalizeLimit(limit);
  const url = new URL(`${String(baseUrl).replace(/\/+$/u, "")}/release-group`);
  url.searchParams.set("query", escapeLucene(normalizedQuery));
...
function orderedCandidates(data) {
