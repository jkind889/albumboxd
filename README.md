...
- `MUSICBRAINZ_CONTACT`: a monitored email address or project URL. Rescened builds the header as `Rescened/<version> (<contact>)`.
...
Neighborhood `cacheStatus` values are `miss` for the first snapshot, `hit` while it is fresh, `refreshed` after the weekly update, and `stale` when an expired snapshot is served during a provider failure.
